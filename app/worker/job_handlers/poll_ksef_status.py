...
class PollKSeFStatusJobHandler:
    ...
    def handle(self, payload: dict) -> None:
        ...
        except (KSeFClientError, Exception) as exc:
            logger.warning(
                "poll_ksef_status: błąd dla transmisji %s: %s",
                transmission_id,
                exc,
            )
...
class PollKSeFStatusJobHandler:
    ...
    def handle(self, payload: dict) -> None:
        ...
        try:
            ...
            logger.warning(
                "poll_ksef_status: błąd KSeF dla transmisji %s: %s",
                transmission_id,
                exc,
            )
            self._schedule_retry(transmission_id, reference_number)
            ...
            logger.exception(
                "poll_ksef_status: nieoczekiwany błąd dla transmisji %s.",
                transmission_id,
            )
            self._schedule_retry(transmission_id, reference_number)
            return
        ...
        if code == 200:
            ...
            # Nieznany status — spóbuj później
            self._schedule_retry(transmission_id, reference_number)
            return
